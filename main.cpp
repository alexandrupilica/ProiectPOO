...
BigInt::BigInt(std::string s)
{
	...
	length = s.length();
	if (length > MAXDIGITS) throw ("OVERFLOW");
	if(length)
...
BigInt& operator-(BigInt &number)
{
	if (number.sign == 1) number.sign = 0;
		else number.sign = 1;
...
BigInt& BigInt::operator+=(BigInt &number)
{
	...
	if (t)
	{
		if (this->GetLength() == MAXDIGITS) throw ("OVERFLOW");
...
BigInt& BigInt::operator-=(BigInt &number)
{
	...
	int m = number.GetLength();
	if (this->GetLength() < number.GetLength()) throw("UNDERFLOW");
...
BigInt& BigInt::operator*=(BigInt &number)
{
	...
	for(i = 0; i < this->GetLength(); i++)
		...
		{
			if (i + j >= MAXDIGITS) throw ("OVERFLOW");
	...
	for (this->GetLength(); this->GetLength() > 1 && !v[this->GetLength() - 1]; this->GetLength()--)
		for (i = 0; i < this->GetLength(); i++) this->Digits[i] = v[i];
...
BigInt& BigInt::operator/=(BigInt &number)
{
	...
	for (i = 0; i < ResLength; i++) this->Digits[i] = result[ResLength - 1 - i];
	return *this;
}
...
BigInt& BigInt::operator/=(const int &integer)
{
	...
	{
		t = t * 10 + this->Digits[i];
		for (AnotherDigit = 9; AnotherDigit*number > t; AnotherDigit--); t -= AnotherDigit * number;
		result[ResLength++] = AnotherDigit;
	}
	this->GetLength() = ResLength;
	for (i = 0; i < ResLength; i++) this->Digits[i] = result[ResLength - 1 - i];
...
{
    if(number1.sign!=number2.sign)return false;
        else
        {
            if(number1.length!=number2.length)return false;
                else
                {
                    for(int i=number1.length-1; i>=0; i--)
                        if(number1.Digits[i]!=number2.Digits[i])return false;
                }
        }
    return true;
}
...
{
    int i;
    if(number1.sign<number2.sign)return true;
    if(number1.sign>number2.sign)return false;
